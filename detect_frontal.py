...
with open("frontal_face_data/train_frontalFace.csv", "r") as input_file:
...
X_test = []
y_test = []
with open("frontal_face_data/test_frontalFace.csv", "r") as input_file:
	read_file = csv.reader(input_file, delimiter='\t')
	for line in read_file:
		#poo = [] 
		#poo.append(line[0])
...
model = BuildRCNN(1, 100, 100, num_classes, 5, 128, 3)
...
if MODE == 'train':
	...
	model.fit_generator(train_gen, steps_per_epoch=X_t.shape[0]/8, epochs=ep, 
		...
		ModelCheckpoint('checkpoint_best_frontal.hdf5',
	...
	if len(saved_weights) == 0:
		...
		model.load_weights('checkpoint_best_frontal.hdf5')
